...
// Playlist class definition for CMPT 225 assignment 1 (Part 1)
// Author: Peter Phuc
// Date: September 2024
...
public:
	...
	PlayListNode(Song sng, PlayListNode* nxt) : song(sng), next(nxt) {} // initializer list 
...
public:
	...
	// PARAM: pl - PlayList to be copied (copy constructor)
	// POST: new PlayList is created that is a deep copy of pl
	PlayList(const PlayList& pl);
	
	// POST: dynamic memory associated with object is deallocated (destructor)
	~PlayList();
...
private:
	// TO DO
	unsigned int song_list; // The number of songs in the playlist
