...
private:
	...
	class UndoStack {
	private:
		UndoCommand* stack; // dynamic array
		int capacity; // current cap of stack
		int top; // top of stack

		// helper function to double stack cap when needed
		void extendStack() {
			capacity *= 2;
			UndoCommand* newStack = new UndoCommand[capacity];
			for (int i = 0; i < top; ++i) {
				newStack[i] = stack[i];
			}
			delete[] stack;
	...
	public:
		...
		// constructor to initialize the undo stack
		UndoStack() {
			capacity = 4;
    		top = 0;
			stack = new UndoCommand[capacity];
		}

		// destructor to clean stack memory
		~UndoStack() {
			delete[] stack;
		}

		// push a new command onto the stack
		void push(const UndoCommand& execute) {
			if (top == capacity) {
				extendStack();
			}
			stack[top++] = execute;
    	...
    	// pop the top command from the stack and return it
		UndoCommand pop() {
			if (top > 0) 
			{
				return stack[--top];
			...
			{
        		return UndoCommand{"", -1, ""}; // Default empty action
			}
		}
